...
const Sunburst = ({ addSelectedItem }) => {
  ...
  const fetchSunburstData = useCallback(async (parentId = null, isInitialLoad = false) => {
    ...
    try {
      ...
      console.log(`Data received for Parent ID: ${parentId ? parentId : 'Initial'}. Type: ${typeof data}, IsArray: ${Array.isArray(data)}`);

      if (parentId) { // Merging children (expecting array C+G)
        if (!Array.isArray(data)) {
             console.error(`API error: Expected an array of children for parent ${parentId}, but got:`, typeof data);
             throw new Error(`API returned incorrect data type for children of ${parentId}`);
        }
        setGraphData(prevData => {
          if (!prevData) {
              console.warn("Cannot merge children, previous graphData is null.");
              return null;
          }
      ...
      } else { // Initial load (expecting root object)
         if (typeof data !== 'object' || data === null || Array.isArray(data)) {
            console.error("API error: Expected a root object for initial load, but got:", typeof data);
            throw new Error("API returned incorrect data type for initial load");
         }
    ...
    if (graphData && svgContainerRef.current) {
      ...
      if (svgNodeRef.current) {
        try {
            ...
            console.warn("Attempted to remove SVG node that might already be gone:", e);
        }
      }
    ...
    return () => {
      ...
      if (svgNodeRef.current && svgContainerRef.current) {
         try {
             ...
             console.warn("Cleanup: Attempted to remove SVG node that might already be gone:", e);
         }
      }
   ...
   */
  const handleNodeClick = useCallback((event, d) => { // d = D3 hierarchy node
      console.log(`--- handleNodeClick (Arc) ---`);
      console.log(`Node Clicked: ID=${d.data._id}, Label=${d.data.label}, Depth=${d.depth}`);
      console.log(`_hasChildren=${d.data._hasChildren}, d.children exists=${!!d.children}`);

      // Determine if the *next* level needs loading.
      let needsLoad = false;
      if (d.data._hasChildren) { // Backend indicates children exist
          if (!d.children) {
              needsLoad = true;
              console.log(` -> Needs Load: Yes (d.children missing despite _hasChildren=true)`);
          } else {
              // Check if any child C needs its children G loaded.
              for (const child of d.children) {
                  if (child.data._hasChildren && !child.children) {
                      needsLoad = true;
                      console.log(` -> Needs Load: Yes (Child ${child.data._id} needs grandchildren loaded)`);
                      break;
                  }
              }
              if (!needsLoad) console.log(` -> Needs Load: No (All necessary grandchildren seem loaded)`);
          }
      } else {
  ...
  }, [fetchSunburstData, isLoading]); // Dependencies
  ...
  const handleCenterClick = useCallback(() => {
      ...
      const centeredNode = currentHierarchy.find(node => node.data._id === currentCenterId);

      if (!centeredNode) {
           console.warn(` Could not find node ${currentCenterId} in current D3 hierarchy. Resetting to root.`);
      ...
      if (centeredNode.parent) {
          ...
          const parentId = centeredNode.parent.data ? centeredNode.parent.data._id : null;
          ...
          console.log(` Zooming Out: Clicked node ${currentCenterId} has no parent. Setting center to Root (null).`);
          setZoomedNodeId(null); // Update React state to the top root
      }
   ...
   */
  const handleSunburstClick = useCallback((e, dataNode) => { // dataNode = D3 hierarchy node
  ...
  return (
    <div className="sunburst-component-wrapper">
      ...
      <div
        ...
        style={{ position: 'relative', minHeight: '600px', width: '100%', maxWidth: '928px' }}
      >
      ...
      {popupVisible && clickedItem && (
        ...
        >
          {/* Popup Content */}
          <p style={{ margin: '0 0 5px 0', fontWeight: 'bold', borderBottom: '1px solid #ccc', paddingBottom: '3px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {clickedItem.label || clickedItem._id} {/* Display label or fallback to ID */}
          </p>
          <a
