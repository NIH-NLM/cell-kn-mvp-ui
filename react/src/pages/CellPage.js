...
const CellPage = ({ match, history }) => {
  ...
  useEffect(() => {
    ...
    // Get outbound edges
    Promise.all([
      getEdges(`${collection}-CL`, collection, true),
    ...
    // Get inbound edges
    Promise.all([
      getEdges(`${collection}-CL`, collection, false),
  ...
  let getCell = async () => {
    let response = await fetch(
      `/arango_api/collection/${collection}/${cellId}/`,
    );
  ...
  let getEdges = async (edge_coll, node_coll, isFrom) => {
    let response = await fetch(
      `/arango_api/edges/${edge_coll}/${isFrom ? "_from" : "_to"}/${node_coll}/${cellId}/`,
    );
    return response.json();
  };
